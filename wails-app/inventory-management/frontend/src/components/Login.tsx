...
export const Login = () => {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch("http://localhost:37285/api/login", {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                ...
                // Store token in localStorage for Wails app
                if (result.token) {
    ...
    return (
        <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
            <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                <div className="mb-4 flex gap-4 text-sm justify-center">
                    <a href="/" className="text-blue-600 hover:underline">← Home</a>
                    <a href="/signup" className="text-blue-600 hover:underline">Sign Up</a>
                    <a href="/debug" className="text-gray-500 hover:underline">Debug</a>
                </div>
                ...
                <p className="mt-8 text-center text-sm text-gray-500">
                    Not a member?{' '}
                    <a href="/signup" className="font-semibold leading-6 text-black hover:text-[#434343]">
