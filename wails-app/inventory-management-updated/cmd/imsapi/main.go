...
func main() {
	...
	salesReportService := service.NewSalesReportService(db)
	productAnalyticsService := service.NewProductAnalyticsService(db)
	abcAnalysisService := service.NewABCAnalysisService(db)
	inventoryTurnoverService := service.NewInventoryTurnoverService(db)
	...
	// Initialize Square service and controller
	...
	// Initialize Terminal service and controller
	...
	// Initialize Sale service (depends on Terminal service)
	...
	saleController.RegisterSaleRoutes(e, middleware.AuthMiddleware, middleware.AdminOnlyMiddleware)
	settingController.RegisterSettingRoutes(e)
	backupController.RegisterBackupRoutes(e, middleware.AuthMiddleware)
	reportController.RegisterReportRoutes(e)
	squareController.RegisterSquareRoutes(e)
	terminalController.RegisterTerminalRoutes(e)
	webhookController.RegisterWebhookRoutes(e)
