...
const navigationItems: NavItem[] = [
    ...
    },
    {
        ...
        icon: ChartBarIcon,
        children: [
            { name: 'Sales Performance', path: '/reports/sales-performance', icon: ChartBarIcon },
            { name: 'Product Analytics', path: '/reports/product-analytics', icon: ChartBarIcon },
            { name: 'ABC Analysis', path: '/reports/abc-analysis', icon: ChartBarIcon },
        ],
    },
    {
        name: 'Settings',
        icon: CogIcon,
        children: [
            { name: 'General Settings', path: '/settings', icon: CogIcon },
            { name: 'Square Setup', path: '/settings/square', icon: CreditCardIcon },
            { name: 'Terminal Devices', path: '/settings/terminal-devices', icon: CreditCardIcon },
        ],
    },
];
...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
    ...
    const { canAccessSquareSettings } = useAuth();
    ...
    const getFilteredNavigationItems = () => {
        return navigationItems.map(item => {
            if (item.name === 'Settings' && item.children) {
                // Filter out Square-related settings for non-admin users
                const filteredChildren = item.children.filter(child => {
                    if (child.name === 'Square Setup' || child.name === 'Terminal Devices') {
                        return canAccessSquareSettings();
                    }
                    return true;
                });
