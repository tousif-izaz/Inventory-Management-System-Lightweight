...
import { getTerminalDevices, createTerminalCheckout } from '../../services/terminalService';
import { TerminalDevice } from '../../types/terminal';
import { TerminalCheckoutModal } from './TerminalCheckoutModal';
...
export const SaleForm = ({ onSuccess, onCancel }: SaleFormProps) => {
    ...
    const [currentCheckoutId, setCurrentCheckoutId] = useState<number | null>(null);
    ...
    // Load terminal devices when square_terminal is selected
    useEffect(() => {
        const loadTerminalDevices = async () => {
            if (formData.payment_method === 'square_terminal') {
                try {
                    // Get merchant ID from settings or Square OAuth
                    const squareConfig = await api.get<any>('/square/config');
                    if (squareConfig.merchant_id) {
                        const devices = await getTerminalDevices(squareConfig.merchant_id);
                    ...
                    console.error('Failed to load terminal devices:', error);
                    showToast('warning', 'Could not load terminal devices. Please configure Square Terminal in Settings.');
                }
            }
        };
        loadTerminalDevices();
    ...
    const validateForm = (): boolean => {
        ...
        if (formData.sale_date) {
            ...
            const now = new Date();
            if (saleDate > now) {
                showToast('error', 'Sale date cannot be in the future');
                return false;
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validate Square Terminal requirements
        if (formData.payment_method === 'square_terminal' && terminalDevices.length === 0) {
            showToast('error', 'Please configure a Square Terminal device in Settings before using Square Terminal payments');
            return;
        }
        ...
        try {
            ...
            // If Square Terminal payment, create checkout
            if (formData.payment_method === 'square_terminal') {
                try {
                    const checkoutResponse = await createTerminalCheckout({
                        sale_id: sale.sale_id,
                        device_id: selectedDeviceId,
                    });

                    // Show checkout modal
                    setCurrentCheckoutId(checkoutResponse.checkout.checkout_id);
                    setShowCheckoutModal(true);
                } catch (checkoutError) {
                    console.error('Failed to create terminal checkout:', checkoutError);
                    const errorMessage = checkoutError instanceof Error ? checkoutError.message : 'Failed to create terminal checkout';
                    showToast('error', `Terminal checkout failed: ${errorMessage}`);
                    setLoading(false);
                    return;
                }
            } else {
    ...
    const handleCheckoutClose = () => {
        ...
        setCurrentCheckoutId(null);
        setLoading(false);
        // Don't call onSuccess here - let user decide what to do
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {formData.payment_method === 'square_terminal' && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <Select
                        ...
                        options={[
                            { value: '', label: terminalDevices.length === 0 ? 'No devices available' : 'Use default device' },
                            ...terminalDevices.map((device) => ({
                                value: device.device_id.toString(),
                                label: `${device.device_name}${device.is_default ? ' (Default)' : ''}`,
                            })),
                        ]}
                    />
                    {terminalDevices.length === 0 && (
                        <p className="text-sm text-orange-600 mt-2">
                            No terminal devices configured. Please add a device in Settings before using Square Terminal payments.
                        </p>
                    )}
                </div>
            )}
            ...
            {showCheckoutModal && currentCheckoutId && (
                <TerminalCheckoutModal
